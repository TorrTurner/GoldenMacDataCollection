...
# --- CONFIGURATION ---
SHEET_NAME = "QC Time Study DB" # Change this if your Google Sheet is named differently
...
def get_gsheet_client():
    """Authenticates using the secret JSON securely stored in Streamlit Cloud"""
    creds_dict = json.loads(st.secrets["GOOGLE_CREDS"])
    scopes = ['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive']
    creds = Credentials.from_service_account_info(creds_dict, scopes=scopes)
    return gspread.authorize(creds)

def log_timestamp(bin_num, style, step_name):
    ...
    # Push to cloud
    sheet.append_row([bin_num, style, step_name, current_time])

# ==========================================
# 2. STATE RECOVERY (Mobile Persistence)
# ==========================================
...
# Phase 2: Recording sequence
    ...
    st.info(f"**Active Bin:** {st.session_state.bin_number} | **Style:** {st.session_state.style}")
    ...
    if st.button(f"⏺ RECORD {current_step_name}", type="primary", use_container_width=True):
        with st.spinner("Saving to Google Sheets..."):
            log_timestamp(st.session_state.bin_number, st.session_state.style, current_step_name)
        st.session_state.current_step += 1
        sync_state_to_url()
        st.rerun()
